...
function jiggy_HFP_volcano(skip)
    ...
    if ( can_longJump() and top <= logictype.CurrentStage ) then
        logic = 0
    elseif ( top <= logictype.CurrentStage and (can_longJump() or has("tjump") or has("splitup")) ) then
        logic = 1

    -- Sequence Breaking
    elseif ( can_longJump() or has("tjump") or has("splitup") ) then
...
function jiggy_HFP_colosseumSplitUp(skip)
    ...
    elseif ( has("splitup") and has("ggrab") and has_packWhack() and has("tjump") ) then
        logic = 1
    elseif ( can_shootEggs("ceggs") and has("splitup") and has("eggshoot") and has("tjump") ) then
        logic = 3
    end
...
function treble_HFP(skip)
    ...
    elseif ( top < 2 and can_shootEggs("geggs") and has("eggaim") and has("tjump") and (has("ttrot") or has("splitup")) ) then
        logic = 1 -- Sequence Breaking
    elseif ( top <= logictype.CurrentStage and can_clockworkShot() and has("eggaim") and (has("ttrot") or has("tjump") and has("splitup")) or extremelyLongJump <= logictype.CurrentStage and can_clockworkShot() ) then
        logic = 2

    -- Sequence Breaking
    else
        local fromTop = 99
        if ( (can_shootEggs("geggs") and has("eggaim") or can_clockworkShot()) and has("eggaim") and (has("ttrot") or has("tjump") and has("splitup")) ) then
            fromTop = math.max(2, top)
        end
...
function nests_HFP_icicleGrottoTop(skip)
    ...
    elseif ( top < 2 and can_shootEggs("geggs") and has("eggaim") and has("tjump") and (has("ttrot") or has("splitup")) ) then
        logic = 1 -- Sequence Breaking
    elseif ( top <= logictype.CurrentStage and can_clockworkShot() and has("eggaim") and (has("ttrot") or has("tjump") and has("splitup")) or extremelyLongJump <= logictype.CurrentStage and can_clockworkShot() or has("clawbts") and pairCube <= logictype.CurrentStage ) then
        logic = 2
...
function jinjo_HFP_icicleGrotto(skip)
    ...
    if ( top <= logictype.CurrentStage and has_glide() and can_shootEggs("geggs") and has("eggaim") and has("tjump") ) then
        logic = 0
    elseif ( top <= logictype.CurrentStage and (has_glide() or has_legSpring() and has_wingWhack()) ) then
        logic = 1
    elseif ( top <= logictype.CurrentStage and (has_legSpring() or can_clockworkShot() and (has("tjump") and has("splitup") or has("ttrot"))) ) then
        logic = 2
...
function honeycomb_HFP_volcano(skip)
    ...
    if ( top <= logictype.CurrentStage and can_shootEggs("geggs") and has("eggaim") and has("tjump") and (has("ttrot") or has("splitup")) ) then
        logic = 0
    elseif ( top <= logictype.CurrentStage and (has("splitup") and (has("tjump") or has_wingWhack() or has_glide() or has_legSpring()) or can_shootEggs("geggs") and has("eggaim") and has("tjump") and has("ttrot")) ) then
        logic = 1
    elseif ( top < 2 and can_shootEggs("geggs") and has("eggaim") and has("tjump") and (has("ttrot") or has("splitup")) ) then
        logic = 1 -- Sequence Breaking
    ...
    -- Sequence Breaking
        ...
        local simple = 99
        if ( can_shootEggs("geggs") and has("eggaim") and has("tjump") and has("ttrot") ) then
            simple = top
        end
...
function cheato_HFP_colosseum(skip)
    ...
    --[[        cheato_colosseum
    if self.intended_logic(state):
        return self.long_jump(state) and self.claw_clamber_boots(state) and (self.has_explosives(state) or self.dragon_kazooie(state))
    elif self.easy_tricks_logic(state):
        return (self.long_jump(state) or self.small_elevation(state)) and self.claw_clamber_boots(state) and (
                    self.has_explosives(state)
                    or state.has(itemName.MUMBOHP, self.player) and self.tall_jump(state)
                    or self.dragon_kazooie(state)
                )
    elif self.hard_tricks_logic(state):
        return self.claw_clamber_boots(state) and \
                (self.has_explosives(state) or
    ...
    if ( can_longJump() and has("clawbts") and (explosives <= logictype.CurrentStage or dragonKazooie <= logictype.CurrentStage) ) then
        logic = 0
    elseif ( (can_longJump() or can_reachSmallElevation()) and has("clawbts") and (explosives <= logictype.CurrentStage or dragonKazooie <= logictype.CurrentStage or has("mumbohp") and has("tjump")) ) then
        logic = 1
    elseif ( (can_longJump() or can_reachSmallElevation()) and has("clawbts") and (explosives < 2 or dragonKazooie < 2) ) then
        logic = 1 -- Sequence Breaking
